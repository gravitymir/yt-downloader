...
use downloader::{
    ensure_ytdlp_available, install_deno, resolve_deno_exe_path, set_cookies_file, update_ytdlp,
    StreamInfo, YoutubeDownloader,
};
...
pub async fn run(port: u16) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Bind `port`, then port+1/port+2 if busy — the Chrome extension probes the same 3.
...
/// Write a tiny plain-text HTTP response (used for cookie-save and file-serving errors).
