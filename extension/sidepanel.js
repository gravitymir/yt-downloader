// Side panel: read Chrome's YouTube cookies, hand them to the local server,
...
// Grab the current tab's YouTube video URL, if any.
async function currentVideoUrl() {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    const url = tab && tab.url ? tab.url : "";
    if (/^https?:\/\/(www\.)?youtube\.com\/watch\?/.test(url)) return url;
    if (/^https?:\/\/(www\.)?youtube\.com\/shorts\//.test(url)) return url;
    if (/^https?:\/\/youtu\.be\//.test(url)) return url;
  } catch (e) { /* no tabs permission or no active tab */ }
  return null;
}
...
async function loadApp() {
  const url = await currentVideoUrl();
  appFrame.src = SERVER + "/" + (url ? "?url=" + encodeURIComponent(url) : "");
  hideError();
}
...
async function findServer() {
  for (const p of PORTS) {
    ...
    try {
      const r = await fetch(base + "/deno-status", { method: "GET" });
      if (r.ok) return base;
